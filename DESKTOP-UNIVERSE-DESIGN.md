...
> **Implementation update — 2026-07-15 (Option B shipped; shell v0.3.0).** The universe is built, and this doc's assumption of a *kernel-served webview* is now historical. **Current architecture:** the UI is **bundled into the app as Tauri assets** (`frontendDist` = `../../ui`, origin `tauri://localhost`), and the kernel is a **pure loopback API**, untouched — its deliberate no-CORS posture is intact. The shell reaches it through a Rust `kernel_request` proxy command, and a `window.fetch` override (injected as an init script, before any page script) routes the hand-written pages' existing calls through it — no page rewrites. The window is **frameless** with a custom Zade titlebar; a `ZADE_DEV_UI` env var gives a live, no-rebuild dev loop (loads `/ui` straight from the kernel); and the shell fires **native OS toasts** for new unread notifications when the window is unfocused. Shipped, verified, resident. The North Star (§1) and the shell decision (§5 — still Tauri) stand; read §4's diagram and §8's Phase-1 kernel-served handoff as the *original plan*, superseded by this note.
...
3. **Panels/apps** — ledger, vault browser, research/daydream, browser-flow console, approvals — as docked surfaces in one world (this is also the missing UI for the browser/vault/research builds). *Ops-group consoles shipped: Brief, Swarm (roles + delegation), Screen, Browser, Vault, Research — each a page reachable from the sidebar + palette.*
...
- **Phase 2 (the universe, ongoing): the world.** Command palette, home-as-place, the panels (incl. the missing browser/vault/research UIs), notification cards, theme/motion/identity, digests. *This is where "universe" is won.*
  - **First slice BUILT + LIVE-VERIFIED 2026-07-13:** command palette (Ctrl+K/Cmd+K in `zade-ui.js` — go-anywhere across all 16 pages + capture-to-memory) and the three Ops panels: `ui/browser.html` (flow composer → L3 queue), `ui/vault.html` (browse/search/trash + dry-run move/delete → L2 queue), `ui/research.html` (topics/daydream/gated run). Sidebar gained an "Ops" group. **Home-as-place BUILT + LIVE-VERIFIED 2026-07-14** — `ui/index.html` gained: a today-strip under the greeting (attention count, unread, pending approvals, model warmth — all live reads, hot items highlighted and linked), a "While you were away" rail digest (unread notifications), "Recently decided" (decision memos, truthful empty state), and "Set me loose" quick actions (inline Daydream POST + links to the Ops panels, Ctrl+K hint). All ambient loads are GETs; the only POST is the explicit Daydream button.
  - **Notification cards BUILT + LIVE-VERIFIED 2026-07-14** — a global notification center in `zade-ui.js`, so notifications are first-class on every page: a bell + live unread badge in the sidebar (and the narrow-screen strip), opening a panel of actionable cards (severity stripe, title/body, relative time, per-card mark-read + mark-all-read). Badge polls `/tray/state`; the panel pulls `/notifications` on open (unread-first). Verified: badge showed the real unread count, mark-read decremented badge + card list and persisted server-side, present on every page.
  - **Theme/motion identity BUILT + LIVE-VERIFIED 2026-07-14** — the universe now moves as one hand. Canonical motion tokens in `:root` (`--z-ease` = the signature curve `zrise` already used, plus `--z-ease-out`, `--z-dur-fast/base/slow`), and both summon surfaces got a signature entrance: the command palette rises in (`z-summon` + `z-scrim` scrim fade), the notification center drops from the bell (`z-drop` + list fade) — same curve, same 0.28s. Builds on the existing character layer (ambient breathe, rotating first-person presence lines, page-entrance `zrise`, cursor-glow shipped OFF). Reduced-motion is honored by the pre-existing global `prefers-reduced-motion` block. Verified live: tokens resolve, both overlays animate with the right keyframes, identity layer live (ambient + "Awake. I don't sleep." presence), console clean.
  - **Digests BUILT + LIVE-VERIFIED 2026-07-14** — `ui/digest.html` ("The brief"): a calm reading surface, not an admin panel. Time-aware greeting in Zade's voice (morning/afternoon/evening/late, addressed to Ellie), then an ambient GET-only assembly — the one thing that matters + severity counts (GET /surface/attention), "Where things stand" (GET /brief/daily), "The founder view" (GET /founder/brief). The only POST is an explicit **Save this brief** (POST /surface/brief, narrate/force options) which writes a durable memory + notification — honoring the ambient-loads-are-GETs-only rule. Added to the sidebar Ops group ("Brief") + palette. Verified live: greeting correct, real one-thing + counts (7 on board / 1 urgent), both prose briefs loaded, console clean, and Save created memory #37 + notification #11 (topic surfacing.brief) — which now also surfaces as a card in the notification center. **Phase 2 planned surfaces are complete** (palette, panels, home-as-place, notification cards, motion identity, digests).
- **Phase 3 (distribution): the product. ✅ CORE SLICE BUILT + LIVE-VERIFIED 2026-07-14** (shell v0.2.0) — **autostart** (resident: boots with Windows via tauri-plugin-autostart, enabled once on first run via a `.autostart-initialized` marker so a later tray-off sticks; login-boot passes `--start-hidden` to come up quiet in the tray; tray *Start with Windows* toggle) · **L2 immersive mode** (Ctrl+Alt+F or tray *Immersive mode* → true full-screen, chrome + taskbar stripped; verified both directions via desktop screenshot) · **Ollama-aware tray tooltip** (15s status thread reads /health → online / brain-offline·start-Ollama / waking). Verified: autostart Run key "Zade"→exe --start-hidden written on first run, marker created, kernel still spawned/healthy, Ctrl+Alt+Z summon + Ctrl+Alt+F immersive both work on the new build.
  - **Phase 3 COMPLETED 2026-07-15 (v0.4.0):** the two initially-deferred items are now done. **Code signing** — `shell/scripts/sign-shell.ps1` Authenticode-signs exe + installer (self-signed `CN=Zade Local Shell` by default; `ZADE_SIGN_THUMBPRINT` swaps a real cert). **Auto-update** — `tauri-plugin-updater` + generated signing keypair (`createUpdaterArtifacts` → signed `.sig`) + tray *Check for updates*, polling a local channel served by the kernel at `GET /shell/latest.json` (loopback → `dangerousInsecureTransportProtocol`; inert version 0.0.0 by default; drop `data_dir/shell-update.json` to publish, or point endpoints at GitHub for remote). **Ollama managed** — auto-starts Ollama when installed-but-down; tray *Install / start Ollama* installs via winget / opens download when missing. Live-verified: v0.4.0 exe launches clean (updater plugin loads), signed installer `Zade_0.4.0_x64-setup.exe` + `.sig` produced, signature present (self-signed = UnknownError until trusted, expected). Gotcha: the updater rejects http endpoints unless `dangerousInsecureTransportProtocol` is set — required for the loopback manifest.
- **Architecture relocation (Option B) + chrome/dev/notifications — SHIPPED + VERIFIED 2026-07-15 (shell v0.3.0):** the UI moved from a kernel-served webview to **bundled Tauri assets**; the kernel is now a pure API (untouched, no-CORS) reached via a Rust `kernel_request` loopback proxy + a `window.fetch` init-script bridge (the ~185 existing `fetch` call sites unchanged). Added **frameless custom chrome** (decorations off + a Zade titlebar: drag region + min / maximize / close-to-tray; hides in immersive), a **live dev loop** (`ZADE_DEV_UI` → loads `/ui` from the kernel, edit + F5, no rebuild; keeps OS chrome and suppresses the custom bar off the asset origin), and **native OS toasts** (a watcher polls `/tray/state`, dedups by id, fires when the window is unfocused). Verified end-to-end via the kernel access log + founder confirmation; `shell/README.md` and this doc refreshed. Also settled: "full system resources" = **compute** (CPU/GPU/RAM) — nothing in code or shell throttles it; the one lever, `OLLAMA_MAX_LOADED_MODELS` 1→2, is set (effective on Ollama restart).
