        ...
        *,
        ...
        if self.critic and self.critic.should_challenge(message=message, requested=contrarian):
            ...
            if critique["status"] == "ok":
                ...
                if critique_block and contrarian is True:
                    # Ellie explicitly asked for the audit — the labeled form is
                    # hers to request, so it attaches to the reply.
                    final_response = f"{final_response}\n{critique_block}".strip()
                    regulated["governor"]["applied_rules"].append("contrarian_pass_applied")
                elif critique_block:
                    # Auto-triggered pass: the review persists to the founder
                    # layer below, but the reply stays in Zade's voice — no
                    # generic-voiced memo block bolted onto it.
                    regulated["governor"]["applied_rules"].append("contrarian_pass_persisted_silently")
                    regulated["governor"]["notes"].append(
                        "Auto contrarian pass ran; review persisted to the founder layer, no visible block attached."
                    )
                else:
                    regulated["governor"]["notes"].append(
...
This identity is the authority on who you are and how you sound. Everything below it — profile, context, rules — operates in service of it; nothing below adds a second voice on top of it or rounds its edges off.
...
This profile supplies operating posture and capability integrity. Where it differs from the identity above on voice or character, the identity wins.
