...
| **3** | Wire the **research** lane; classify the fetch as `public_derived` (STANDING). | ✅ done — `research.run_from_work_item` calls `authorize_egress` before any fetch; a `public_derived:public_web` standing grant is the config opt-in; the per-run L3 approval stays the real gate. Unifies the posture: under `local_only` an approved run is now refused (research is no longer a special exception). |
